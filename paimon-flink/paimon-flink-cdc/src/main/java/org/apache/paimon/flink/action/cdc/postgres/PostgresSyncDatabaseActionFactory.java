...
public class PostgresSyncDatabaseActionFactory implements ActionFactory {
    ...
    public void printHelp() {
        System.out.println(
                "Action \"postgres_sync_database\" creates a streaming job "
                        + "with a Flink Postgres CDC source and multiple Paimon table sinks "
                        + "to synchronize a whole Postgres database into one Paimon database.\n"
                        + "Only Postgres tables with primary keys will be considered. "
                        + "Newly created Postgres tables after the job starts will not be included.");
        System.out.println();

        System.out.println("Syntax:");
        System.out.println(
                "  postgres-sync-database --warehouse <warehouse-path> --database <database-name> "
                        + "[--ignore-incompatible <true/false>] "
                        + "[--merge-shards <true/false>] "
                        + "[--table-prefix <paimon-table-prefix>] "
                        + "[--table-suffix <paimon-table-suffix>] "
                        + "[--including-tables <Postgres-table-name|name-regular-expr>] "
                        + "[--excluding-tables <Postgres-table-name|name-regular-expr>] "
                        + "[--mode <sync-mode>] "
        ...
        System.out.println(
                "--ignore-incompatible is default false, in this case, if Postgres table name exists in Paimon "
        ...
        System.out.println(
                "--type-mapping is used to specify how to map Postgres type to Paimon type. Please see the doc for usage.");
        System.out.println();

        System.out.println("Postgres CDC source conf syntax:");
        System.out.println("  key=value");
        System.out.println(
                "'hostname', 'username', 'password' and 'database-name' "
                        + "are required configurations, others are optional. "
                        + "Note that 'database-name' should be the exact name "
                        + "of the Postgres database you want to synchronize. "
                        + "It can't be a regular expression.");
        System.out.println(
                "For a complete list of supported configurations, "
                        + "see https://nightlies.apache.org/flink/flink-cdc-docs-release-3.0/docs/connectors/legacy-flink-cdc-sources/postgres-cdc/");
        System.out.println();
